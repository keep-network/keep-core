...
griefing from malicious nodes (m) if, given a threshold (t), the mali is atleast t ∈ [m+1, n-m]
m ≤ t - 1
...
Through using these implementations, the relay is susceptible to the following
...
At a network level, we ensure that ingress and egress are limited to the ports
required to communicate over libp2p, sync with ethereum, and optionally, to
exchange debug information with us.


===== Slow Down (slowloris), resource exchaustion, etc.

Slowloris: We configure clients such that we limit the number of connections a
...
client is allowed to stay connected
