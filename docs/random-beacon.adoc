...
Hence our mitigations are focused on ensuring the above properties hold for all groups.
...
gets lost (through retrying). At the implementtation layer, we ensure to
differentiate between a lost/missed message and a non-participating group member
through setting deadlines and retries.

Furthermore, the relay is reliant on the Golang standard library (ie. net/http).
Through using these implementations, we discuss the possible network-level attacks:
...
Hmmm...todo: you can still have this happen under authenticated streams.



===== Message Silencing

Astute readers may point out that an attacker could attempt to silence messages
from an honest actor (by crafting messages with conflicting ids on behalf of an
honest actor), but the relay combats message silencing through

  1. being threshold-based where t of n shares are required to provide a
  successful result (the argument here being that the probability of silencing
  the threshold t is equivalent to a malicious owner owning [some value]), but
  more importantly
  2. messages are signed by their authors and addressed by their hashes.
  This means that content (for example, group shares) is hashed. (TODO: wait, does this actually work for dkg lol)


===== Slow Down (slowloris), resource exchaustion, etc.
...
speed a connection is allowed to have, and restricting the length of time a
client is allowed to stay connected. Concretely, this is done through:

  1.Setting http://golang.org/pkg/net/http/#Server[Server.ReadTimeout]. These are deadlines that apply to each request.
  2. Setting 

At a network level, we advise that ingress and egress are limited to the ports
...
At a macro level, we defend against attacks on the network through the following
incentive structure, which was originally mentioned in the whitepaper:

====
Keep  providers  will  be  required  to
...
| R        | tx submitter reward
| G        | group reward
| N        | members in a group
| E        | Invalid signature punishment for the submitter
| K        | Minimum stake amount (in KEEP)
| P        | Aborted group sig punishment
| f        | Function that executes punishment for aborting group sig
|=========

G/N is the share of the group reward any member receives
Δf ∈ [-1, 1] is a real number between -1 and 1, representing a multiplicitive facto
...
Currently an attributable Distributed Key Generation does not exist. Keep is
exploring the possibilities here. For example, consider the a potential attack
where a lazy/malicious actor fails to provide shares or provides malformed
shares to the group. In this case, group members

Simply, a malicious actor with approximately [malicious_node_distribution] nodes
