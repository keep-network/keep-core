...
* _v~r~_ is signed by at least a subset of the chosen group members, and the
  ...
  published to the anchoring blockchain as the entry _v~r~_.
* The new entry _v~r~_ triggers the formation of a new group from the set of
...
include::random-beacon/group-selection.adoc[leveloffset=+2]

include::random-beacon/dkg.adoc[leveloffset=+2]
