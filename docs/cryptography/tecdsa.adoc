...
* `Master Trapdoor` (`x`) generated in a setup phase has to remain private and 
can be known to the Verifier only. It's described in 
...
* Generated `ECDSA Private Key` is too weak (e.g. just 10-bit long). It gives 
a opportunity to brute-force the `ECDSA Private Key` when knowing `ECDSA Public Key`.
