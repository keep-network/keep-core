...
`G_q` is selected. The value `t` is kept secret until the commitment 
verification phase. Finally, the commitment is a result of `g^s * h^t`.
...
expectedCommitment  = [ (g ^ s) % q ] * [ (h ^ s) % q ] % q
