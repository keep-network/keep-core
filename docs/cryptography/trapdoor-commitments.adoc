...
`q` and where `g` is a generator of `G_q`.

Then we choose a random `h` which is an element of `G_q` and that `log_g(h)` is
unknown.
...
To generate a commitment to a message `m`, we first need to map `m` into `G_q`
(this mapped version will be denoted `s`).  We also need to select a random `t`
from `G_q` is selected. The value `t` is kept secret until the commitment
...
value as the original pair `(s, t)`. So even if `log_g(h)` is known to the
...
been published) since it's possible to find another pair of `(t', m')`
evaluating to the same commitment. 

Note that `log_g(h)` is a discrete logarithm problem and if we want to enable
trapdoor, `log_g(h)` must be known at the moment of choosing `g` and `h`.
...
change `m -> m'` (mapped to `G_g` is `s -> s'`) and knowing `log_g(h)`, she can
compute:
