...
The Pedersen commitment requires to have a two large primes `p` and `q` such
that `q` divides `p - 1`, a `G_q` which is a unique subgroup of `Z_p` of order
`q` and `g` is a generator of `G_q`.

Then lets take `h` which is an element of `G_q` and that `log_g(h)` is unknown.

```
q <- is a prime
p = 2q + 1 <- is a prime
g = randomZ[2, q - 1]
h = g ^ randomZ[0, q - 1]) mod q
```

=== Generating commitment
To generate a commitment to a message `m`, first it needs to be represented as
a part of group `G_q` (which we will denote as an `s`), then a random `t` from
`G_q` is selected. Finally, the commitment is a result of `g^s * h^t`.
...
commitment = [ (g ^ s) % q ] * [ (h ^ t) % q ]
```

=== Commitment verification
The commitment verification is analogical to the commitment generation, because
...
same, then we have a successfull commitment verification.
...
expectedCommitment  = [ (g ^ s) % q ] * [ (h ^ s) % q ]
...
There is an arbitrary number of `(m', t')` pairs mapping to the same `commitment` 
value as the original pair `(m, t)`. So even if `log_g(h)` is known to the
...
been published) since it's possible to find another pair of `(s', m')` 
...
message. If one has committed to message `m` with `(m, t)`, then in order to 
