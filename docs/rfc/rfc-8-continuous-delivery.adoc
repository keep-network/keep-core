...
==== Part 1: Contract Migration and ConfigMap Updates
...
value store. The keep-client is then instructed via the `consulHost` commandline
parameter to query Consul for its configuration.
...
==== Part 2: ETH Account Preparation
...
The `InitContainer` will operate exclusively on the ETH account assigned to the
...
for doing unlock and stake.  This will be checked into the
