...
means that the staking contract/s used by the operator contract
must be hard-coded.
Operator Contracts are also aware of the token contract.

Operator contracts can be exclusive or non-exclusive.
Non-exclusive operator contracts provide services
to any contract that makes a valid request and pays the correct fee,
while each exclusive operator contract is owned by a specific contract,
and only serves its owner.
...
===== Exclusive operator contracts

Exclusive operator contracts are tied to an _owner contract_,
only accepting and serving requests from that particular contract. 

Owner contract::
The contract an exclusive operator contract serves.

===== Keep factories

Keep factories are non-exclusive operator contracts
that create keeps for customer applications.
Any contract can request a keep factory to create a keep.
...
Keeps are exclusive operator contracts created by keep factories.
...
instead, they rely on the authorization of their factory.
...
who can activate or deprecate operator contracts.

To activate a operator contract, the following conditions must apply:
...
To deprecate a operator contract, the following conditions must apply:
...
Keep vendors are a type of service contracts
which perform version management of keep factories
by abstracting over upgrades
and providing customers a single unified interface to request up-to-date keeps.
...
. (Optional) Deprecate old operator contracts if needed
...
which operator contract they want to serve their request.
