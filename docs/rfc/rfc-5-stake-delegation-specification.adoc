...
2. The _owner_ provides to the contract a signature of his address, signed by
the _operator_. This proofs that the _operator_ is willing to operate on behalf
of the _owner. It is enough to let the _owner_ to delegate the tokens to the
_operator_. Operator does not have to agree on the number of tokens delegated
and operator's agreement on delegation is given in perpetuity _owner_ may
transfer more tokens to the contract or not delegate all tokens in the first
order.
...
any condition is unfulfilled, processing aborts):
  ...
  * _owner_ address is signed by the _operator_,
