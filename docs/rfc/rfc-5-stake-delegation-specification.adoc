...
Delegating contract should consist of following information:
...
1. The _owner_ chooses the amount of tokens to delegate, _operator_, and
...
5. The _operator_ can now use delegated stake for operating.
...
required to join the operation, after which the following should be checked:
  * the agreement is valid,
  * the state of agreement is `active`,
...
higher then the minimum stake.

3. If the penalty is valid, the _delegation contract_ will substract the 
