...
the network may execute the protocol again with the next bootstrap peer.
...
not communicate for a certain period of time. Nonce timeout is a parameter tracked 
on-chain. Two peers need to execute the handshake protocol again to initialize a 
new nonce if the previous one has expired. If the nonce has exhausted (reached the 
maximum possible value), it should be set back to 0.
