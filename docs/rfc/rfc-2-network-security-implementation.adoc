...
Two peers communicating on the application level does not necessarily need to 
have a direct network-level connection with each other. It is possible messages 
are relayed by other peers. Each peer relying the message validates the message 
signature against sender's key and should drop the message if the signature is 
not valid. This strategy reduces the risk of flooding the network with tampered 
messages.
