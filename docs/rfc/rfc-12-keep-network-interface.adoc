...
Gateway is a smart contract with which the application interacts to obtain the 
most current keep factory version backed by enough stakers so that all 
operations on the created keep can go smoothly. Each gateway is a service 
contract and does not have to be sanctioned by stakers. Each gateway can operate 
on several versions of keep factory to allow for a smooth upgrade process.
...
factories) and gateways. It ensures that only Keep-Org approved contracts are 
used.
...
contract KeepRegistry {
    ...
    function getECDSAKeepGateway() address {
        // (...)
    }
}
```
...
contract Application {
    ...
    function openDeposit() {
        address gateway = KeepRegistry(_keepRegistry).getECDSAKeepGateway()    
        gateway.openBondedECDSAKeep(
            threshold, 
            groupSize, 
            bond
        );
