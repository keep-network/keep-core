...
and the _operator_, _magpie_ and _authorizer_ addresses.
...
* `authorizer` address to determine authorized contracts

stake(amount, operator, magpie, authorizer)::
...
+
May be called by a functionality contract authorized by the staker's authorizer
and not blocked by the panic button

==== Authorization contract

authorized[authorizer] -> [contract_address]::
The contracts authorized by each authorizer
...
for all stakers using the caller as their authorizer
+
