...
Afer executing `install.sh` and `initialize.sh` now you can run `./scripts/start.sh` to start a client. It will prompt you to chose from the available config files that should match the one chosen for `./scripts/initialize.sh`. 
```
./scripts/start.sh
