...
We also need to create a new network entry in `keep-core/contracts/solidity` 
...
Having done all those steps we can finally run a migration and deploy our contracts:
...
deployed instances. Please use addresses of `KeepGroup` and `KeepRandomBeacon` 
proxy contracts as well as the address of `KeepGroup` contract from the truffle 
migration output:
```
[ethereum.ContractAddresses]
        KeepRandomBeacon = "0xb63a1a13a5099fad0fb703d3161ec08d4db2ab83"
        GroupContract = "0xde93fc1fd2dc3679c48dc9abac96b731112f1670"
        KeepGroup = "0xde93fc1fd2dc3679c48dc9abac96b731112f1670"
...
`Seed` value should remain commented out or should be removed.
...
The last thing we need to do is to start each non-bootstrap instance:
```
