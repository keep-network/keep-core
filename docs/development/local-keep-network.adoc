...
=== Install script
First be sure you have created a `src` directory in `$HOME/go/src`
for our go packages if it does not already exist. Then ensure you have
`protoc-gen-gogoslick` already installed and your go `bin` is accessible from your path `/Users/username/go/bin`. To check run `which protoc-gen-gogoslick`.
...
We also need to create a new network entry in `keep-core/solidity/truffle.js`
...
In the `config.local.1.toml` enable *network settings* for bootstrap peer and be
sure to remove your `Peers` section for your first client since no other peers exist
for the first:
```toml
[LibP2P]
        Port = 3919
...
In the base `keep-core` directory create a *storage directory*. Then inside of the
storage directory create directories for each client. Lastly, update your `DataDir` path:
```toml
[Storage]
...
```
$ LOG_LEVEL="info" KEEP_ETHEREUM_PASSWORD="password" ./keep-core --config ./configs/config.local.1.toml start
------------------------------------------------------------------------------------------------
| Port: 3919                                                                                   |
...
```
$ LOG_LEVEL="info" KEEP_ETHEREUM_PASSWORD="password" ./keep-core --config ./configs/config.local.2.toml start
------------------------------------------------------------------------------------------------
| Port: 3920                                                                                   |
...
```
$ LOG_LEVEL="info" KEEP_ETHEREUM_PASSWORD="password" ./keep-core --config ./configs/config.local.3.toml start
------------------------------------------------------------------------------------------------
| Port: 3921                                                                                   |
...
```
$ LOG_LEVEL="info" KEEP_ETHEREUM_PASSWORD="password" ./keep-core --config ./configs/config.local.4.toml start
------------------------------------------------------------------------------------------------
| Port: 3922                                                                                   |
...
```
$ LOG_LEVEL="info" KEEP_ETHEREUM_PASSWORD="password" ./keep-core --config ./configs/config.local.5.toml start
------------------------------------------------------------------------------------------------
| Port: 3923                                                                                   |
...
the required DKG fee. Run the following command from the `keep-core/solidity` directory:
```
...
In order to send the request relay entry the first group must be formed. This process can take some time
but you should eventually see a log something like `INFO keep-relay: [member:7] ready to operate in the group node.go:137`

Once the first group is created we can request relay entry with the following script.
Run the following command from the `keep-core/solidity` directory:
```
