...
describe("LockUtils", () => {
  ...
  it("overwrites existing locks", async () => {
    ...
    const creatorsPre = await locks.publicEnumerateCreators()
    ...
    const creatorsPost = await locks.publicEnumerateCreators()
  ...
  it("reorders locks correctly when deleting", async () => {
    ...
    await locks.publicSetLock(bob, 2)
    const creatorsDoublePost = await locks.publicEnumerateCreators()
    ...
    await locks.publicReleaseLock(alice)
    const creatorsTriplePost = await locks.publicEnumerateCreators()
    expect(creatorsTriplePost[0]).to.equal(bob)
    expect(creatorsTriplePost[1]).to.equal(carol)

    await locks.publicReleaseLock(bob)
    const creatorsQuadPost = await locks.publicEnumerateCreators()
    expect(creatorsQuadPost[0]).to.equal(carol)

    await locks.publicReleaseLock(carol)
    const creatorsPentaPost = await locks.publicEnumerateCreators()
