...
import { ThresholdToken } from "../../../utils/token.utils"
...
const StakeOnThresholdComponent = ({
  ...
  const thresholdTokenAmount = useMemo(() => {
    const keepAmountInBN = new BigNumber(keepAmount)

    return keepAmountInBN.multipliedBy(KEEP_TO_T_EXCHANGE_RATE).toString()
  }, [keepAmount])
        ...
        <List className="mt-2">
          <List.Content className="text-grey-50">
            ...
            <List.Item className="flex row center">
              ...
              <span>1 KEEP = {KEEP_TO_T_EXCHANGE_RATE} T</span>
            </List.Item>
          </List.Content>
        </List>
      </ModalBody>
