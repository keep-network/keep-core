...
func init() {
	...
	BridgeCommand.AddCommand(
		...
		bSetPendingMovedFundsSweepRequestCommand(),
...
func bProcessPendingMovedFundsSweepRequestCommand() *cobra.Command {
	c := &cobra.Command{
		Use:                   "process-pending-moved-funds-sweep-request [arg_walletPubKeyHash] [arg_utxo_json]",
		Short:                 "Calls the nonpayable method processPendingMovedFundsSweepRequest on the Bridge contract.",
		Args:                  cmd.ArgCountChecker(2),
		RunE:                  bProcessPendingMovedFundsSweepRequest,
		SilenceUsage:          true,
		DisableFlagsInUseLine: true,
	}
...
func bProcessPendingMovedFundsSweepRequest(c *cobra.Command, args []string) error {
	contract, err := initializeBridge(c)
	if err != nil {
		return err
	}
	...
	arg_utxo_json := abi.BitcoinTxUTXO{}
	if err := json.Unmarshal([]byte(args[1]), &arg_utxo_json); err != nil {
		return fmt.Errorf("failed to unmarshal arg_utxo_json to abi.BitcoinTxUTXO: %w", err)
	}
	...
	if shouldSubmit, _ := c.Flags().GetBool(cmd.SubmitFlag); shouldSubmit {
		// Do a regular submission. Take payable into account.
		transaction, err = contract.ProcessPendingMovedFundsSweepRequest(
			arg_walletPubKeyHash,
			arg_utxo_json,
		)
		if err != nil {
		...
		// Do a call.
		err = contract.CallProcessPendingMovedFundsSweepRequest(
			arg_walletPubKeyHash,
			arg_utxo_json,
			cmd.BlockFlagValue.Int,
		)
...
func bSetActiveWalletCommand() *cobra.Command {
	c := &cobra.Command{
		Use:                   "set-active-wallet [arg_activeWalletPubKeyHash]",
		Short:                 "Calls the nonpayable method setActiveWallet on the Bridge contract.",
		Args:                  cmd.ArgCountChecker(1),
		RunE:                  bSetActiveWallet,
		SilenceUsage:          true,
		DisableFlagsInUseLine: true,
	}
...
func bSetActiveWallet(c *cobra.Command, args []string) error {
	contract, err := initializeBridge(c)
	if err != nil {
		return err
	}

	arg_activeWalletPubKeyHash, err := decode.ParseBytes20(args[0])
	if err != nil {
		return fmt.Errorf(
			"couldn't parse parameter arg_activeWalletPubKeyHash, a bytes20, from passed value %v",
			args[0],
		)
	}
	...
	if shouldSubmit, _ := c.Flags().GetBool(cmd.SubmitFlag); shouldSubmit {
		// Do a regular submission. Take payable into account.
		transaction, err = contract.SetActiveWallet(
		...
		// Do a call.
		err = contract.CallSetActiveWallet(
			arg_activeWalletPubKeyHash,
			cmd.BlockFlagValue.Int,
		)
		if err != nil {
...
func bSetPendingMovedFundsSweepRequestCommand() *cobra.Command {
	c := &cobra.Command{
		Use:                   "set-pending-moved-funds-sweep-request [arg_walletPubKeyHash] [arg_utxo_json]",
		Short:                 "Calls the nonpayable method setPendingMovedFundsSweepRequest on the Bridge contract.",
		Args:                  cmd.ArgCountChecker(2),
		RunE:                  bSetPendingMovedFundsSweepRequest,
		SilenceUsage:          true,
		DisableFlagsInUseLine: true,
	}
...
func bSetPendingMovedFundsSweepRequest(c *cobra.Command, args []string) error {
	contract, err := initializeBridge(c)
	if err != nil {
		return err
	}
	...
	if shouldSubmit, _ := c.Flags().GetBool(cmd.SubmitFlag); shouldSubmit {
		// Do a regular submission. Take payable into account.
		transaction, err = contract.SetPendingMovedFundsSweepRequest(
			arg_walletPubKeyHash,
			arg_utxo_json,
		)
		if err != nil {
		...
		// Do a call.
		err = contract.CallSetPendingMovedFundsSweepRequest(
			arg_walletPubKeyHash,
			arg_utxo_json,
			cmd.BlockFlagValue.Int,
		)
...
func bSetWalletCommand() *cobra.Command {
	c := &cobra.Command{
		Use:                   "set-wallet [arg_walletPubKeyHash] [arg_wallet_json]",
		Short:                 "Calls the nonpayable method setWallet on the Bridge contract.",
		Args:                  cmd.ArgCountChecker(2),
		RunE:                  bSetWallet,
		SilenceUsage:          true,
		DisableFlagsInUseLine: true,
...
func bSetWallet(c *cobra.Command, args []string) error {
	contract, err := initializeBridge(c)
	if err != nil {
		return err
	}
	...
	arg_wallet_json := abi.WalletsWallet{}
	if err := json.Unmarshal([]byte(args[1]), &arg_wallet_json); err != nil {
		return fmt.Errorf("failed to unmarshal arg_wallet_json to abi.WalletsWallet: %w", err)
	}
	...
	if shouldSubmit, _ := c.Flags().GetBool(cmd.SubmitFlag); shouldSubmit {
		// Do a regular submission. Take payable into account.
		transaction, err = contract.SetWallet(
			arg_walletPubKeyHash,
			arg_wallet_json,
		)
		if err != nil {
...
func bSetWalletMainUtxoCommand() *cobra.Command {
	c := &cobra.Command{
		Use:                   "set-wallet-main-utxo [arg_walletPubKeyHash] [arg_utxo_json]",
		Short:                 "Calls the nonpayable method setWalletMainUtxo on the Bridge contract.",
		Args:                  cmd.ArgCountChecker(2),
		RunE:                  bSetWalletMainUtxo,
		SilenceUsage:          true,
		DisableFlagsInUseLine: true,
	}
...
func bSetWalletMainUtxo(c *cobra.Command, args []string) error {
	contract, err := initializeBridge(c)
	if err != nil {
		return err
	}
	...
	if shouldSubmit, _ := c.Flags().GetBool(cmd.SubmitFlag); shouldSubmit {
		// Do a regular submission. Take payable into account.
		transaction, err = contract.SetWalletMainUtxo(
			arg_walletPubKeyHash,
			arg_utxo_json,
		)
		if err != nil {
		...
		// Do a call.
		err = contract.CallSetWalletMainUtxo(
			arg_walletPubKeyHash,
			arg_utxo_json,
			cmd.BlockFlagValue.Int,
		)
...
func bTimeoutPendingMovedFundsSweepRequestCommand() *cobra.Command {
	c := &cobra.Command{
		Use:                   "timeout-pending-moved-funds-sweep-request [arg_walletPubKeyHash] [arg_utxo_json]",
		Short:                 "Calls the nonpayable method timeoutPendingMovedFundsSweepRequest on the Bridge contract.",
		Args:                  cmd.ArgCountChecker(2),
		RunE:                  bTimeoutPendingMovedFundsSweepRequest,
		SilenceUsage:          true,
		DisableFlagsInUseLine: true,
	}
...
func bTimeoutPendingMovedFundsSweepRequest(c *cobra.Command, args []string) error {
	contract, err := initializeBridge(c)
	if err != nil {
		return err
	}
	...
	if shouldSubmit, _ := c.Flags().GetBool(cmd.SubmitFlag); shouldSubmit {
		// Do a regular submission. Take payable into account.
		transaction, err = contract.TimeoutPendingMovedFundsSweepRequest(
			arg_walletPubKeyHash,
			arg_utxo_json,
		)
		if err != nil {
		...
		// Do a call.
		err = contract.CallTimeoutPendingMovedFundsSweepRequest(
			arg_walletPubKeyHash,
			arg_utxo_json,
			cmd.BlockFlagValue.Int,
		)
