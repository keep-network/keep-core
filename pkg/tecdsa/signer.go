...
// generateDsaKeyShare generates a DSA key shares xi and yi and puts
...
func (s *LocalSigner) InitializeDsaKeyGen() (*InitMessage, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not initialize DSA key generation [%v]", err)
	}

	exi, err := s.paillierKey.Encrypt(keyShare.xi, rand.Reader)
	if err != nil {
		return nil, fmt.Errorf("could not initialize DSA key generation [%v]", err)
	}
...
// each LocalMember of the signing group along with E(xn).
