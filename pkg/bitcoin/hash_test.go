...
func TestComputeHash(t *testing.T) {
	testutils.AssertStringsEqual(
		...
		ComputeHash([]byte{0x01, 0x02, 0x03, 0x04, 0x05}).Hex(InternalByteOrder),
	)
	...
	testutils.AssertStringsEqual(
		...
		ComputeHash([]byte{0xAA, 0xBB, 0xCC, 0xDD, 0xEE}).Hex(InternalByteOrder),
	)
	...
	testutils.AssertStringsEqual(
		...
		ComputeHash([]byte{0x01, 0xAA, 0x02, 0xBB, 0x03}).Hex(InternalByteOrder),
	)
}
