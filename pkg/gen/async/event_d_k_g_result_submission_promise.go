...
// EventDKGResultSubmissionPromise represents an eventual completion of an
...
type EventDKGResultSubmissionPromise struct {
	mutex      sync.Mutex
	successFn  func(*event.DKGResultSubmission)
	failureFn  func(error)
	completeFn func(*event.DKGResultSubmission, error)

	isComplete bool
	value      *event.DKGResultSubmission
	err        error
}
...
func (p *EventDKGResultSubmissionPromise) callCompleteFn() {
	if p.completeFn != nil {
		go func() {
...
func (p *EventDKGResultSubmissionPromise) callSuccessFn() {
	if p.successFn != nil {
		go func() {
			p.successFn(p.value)
		}()
...
func (p *EventDKGResultSubmissionPromise) callFailureFn() {
	if p.failureFn != nil {
		go func() {
			p.failureFn(p.err)
		}()
