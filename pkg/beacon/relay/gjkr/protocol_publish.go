	...
	for !chainRelay.IsResultPublished(result) { // while not resultPublished
		...
		if sliceutils.Contains(publishersIDs, pm.ID) {
			...
			chainRelay.SubmitResult(pm.ID, result).
				OnComplete(func(resultPublish *event.PublishedResult, err error) {
...
func (pm *PublishingMember) determinePublishersIDs() ([]int, error) {
	expectedProtocolDuration := pm.protocolConfig.expectedProtocolDuration // t_dkg

	// Current block height.
	currentBlock, err := pm.protocolConfig.CurrentBlock() // t_now
	if err != nil {
	...
	elapsedBlocks := currentBlock - pm.protocolConfig.initialBlockHeight
	...
	if elapsedBlocks <= expectedProtocolDuration { // if T_elapsed <= T_dkg
		...
		highestMemberIndex = int(math.Ceil(float64(surpassBlocks) / float64(pm.protocolConfig.blockStep)))
