...
func initializePQ() (*big.Int, *big.Int, error) {
	...
	if !result {
		return nil, nil, fmt.Errorf("failed to initialize p")
	}

	q, result := new(big.Int).SetString(qStr, 0)
	if !result {
		return nil, nil, fmt.Errorf("failed to initialize q")
	}
	return p, q, nil
}

func initializeVSS() (*VSS, error) {
	p, q, err := initializePQ()
	if err != nil {
