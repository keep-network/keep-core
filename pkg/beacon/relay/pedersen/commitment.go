...
func GenerateParameters() (*VSS, error) {
...
func Generate(parameters *VSS, secret []byte) (*Commitment, *DecommitmentKey, error) {
...
func (c *Commitment) Verify(parameters *VSS, decommitmentKey *DecommitmentKey, secret []byte) bool {
	digest := calculateDigest(secret, q)
...
func calculateCommitment(parameters *VSS, digest, r *big.Int) *big.Int {
	// ((g ^ digest) % p) * ((h ^ r) % p)
	return new(big.Int).Mul(
		new(big.Int).Exp(parameters.g, digest, p),
		new(big.Int).Exp(parameters.h, r, p),
	)
}
