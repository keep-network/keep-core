...
contract('KeepToken', function(accounts) {
  ...
  it("should not allow to stake to an operator who has KEEP tokens", async function() {
    let amount = web3.utils.toBN(1000000000);
    let vestingDuration = duration.days(60);
    let start = await latestTime();
    let cliff = duration.days(10);
    ...
    // Send KEEP tokens to the operator
    await token.transfer(account_two_operator, amount, {from: account_one});

    // should throw cause the operator has KEEP tokens
