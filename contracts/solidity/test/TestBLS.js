...
contract('TestBLS', function() {
  ...
  let bn;
  beforeEach(async () => {
    bn = await AltBn128.new();
  });

  it("should be able to verify threshold BLS recovered/reconstructed signature", async function() {

    // Data generated using 'keep-core/pkg/bls' with master secret key 123 and message "Hello!"
    let result = await bn.verify(
  ...
  it("should be able to verify BLS aggregated signature", async function() {
    ...
    // Data generated using 'keep-core/pkg/bls' with multiple random signers signing the same message "Hello!"
    let result = await bn.verify(
