...
class StakingDelegateTokenGrantForm extends Component {
  ...
  async submit() {
    ...
    const { web3 } = this.props
    const stakingContractAddress = web3.stakingContractAddress.options.address;    // Operator must sign grantee and token grant contract address since grant contract becomes the owner during grant staking.
