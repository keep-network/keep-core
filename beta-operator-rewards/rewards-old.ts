...
export async function calculateRewardsFactors() {
  ...
  let peersData = new Map<string, any>(); // peer address -> {component name: factor}
  ...
  for (let i = 0; i < (await bootstrapData.data.result.length); i++) {
    ...
    peerData.set('uptime_percent', resultUptimePercent)
    ...
    peerData.set('ip', peer.metric.instance)
    
    let siblings = peersData.get(peer.metric.chain_address) || [];
    siblings.push(peerData);
    console.log(peer.metric.chain_address, siblings)
    peersData.set(peer.metric.chain_address, siblings);
  }
