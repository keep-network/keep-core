...
export async function calculateBetaOperatorFactors() {
  program
    .version("0.0.1")
    .requiredOption(
      "-s, --start <timestamp>",
    ...
    .requiredOption(
      "-a, --api <prometheus api>",
      "prometheus API"
    )
    .requiredOption(
      "-j, --job <prometheus job>",
      "prometheus job"
    )
    .requiredOption(
      "-o, --output <file>",
  ...
  const bootstrapData = await queryPrometheus(queryBootstrapData, paramsBootstrapData);
  ...
  for (let i = 0; i < await bootstrapData.data.result.length; i++) {
    ...
    const upFactorCoefficient = upFactor ? uptimeSearchRange/rewardsInterval : 0
    ...
    const resultPreParams = await queryPrometheus(queryPreParams, paramsPreParams)
    peerData.set(factors.preParams, 1);
    if (resultPreParams.data.result.length == 0) {
      peerData.set(factors.preParams, 0);
    } else {
      resultPreParams.data.result[0].values.forEach(function (peerPreParams: any) {
    ...
    // unix timestamp: git show -s --format=%ct v1.3.1-6464-g43ea52158
