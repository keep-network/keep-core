...
const fetchSlashedTokens = async (web3Context) => {
  ...
  const unauthorizedSigningEvents = await contractService
    .getPastEvents(
      web3Context,
  ...
  const relayEntryTimeoutEvents = await contractService
    .getPastEvents(
      web3Context,
      OPERATOR_CONTRACT_NAME,
  ...
  const slashedTokensGroupedByTxtHash = groupByTransactionHash(slashedTokensEvents)
  const seizedTokensGroupedByTxtHash = groupByTransactionHash(seizedTokensEvents)

  for (let i = 0; i < punishmentEvents.length; i++) {
    const { event, transactionHash, blockNumber, returnValues: { groupIndex } } = punishmentEvents[i]
    let punishmentData = {}
    ...
    punishmentData.date = moment.unix((await eth.getBlock(blockNumber)).timestamp)
