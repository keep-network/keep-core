...
import { SET_TOKENS_CONTEXT, SET_SELECTED_GRANT } from '../reducers/tokens-page.reducer'
...
const TokensContextSwitcher = (props) => {
    ...
    const setTokensContext = useCallback((contextName) => {
        dispatch({ type: SET_TOKENS_CONTEXT, payload: contextName })
    }, [dispatch])

    const onSelectGrant = useCallback((grant) => {
    ...
    return (
        <div className="tokens-context-switcher-wrapper">
            <div
                className={`grants ${tokensContext === 'granted' ? 'active' : 'inactive'}`}
            ...
            >
                <div className="flex row">
                    ...
                    <div className="ml-1">
                        ...
                        <h4 className="balance">{displayAmount(selectedGrant.availableToStake)}</h4>
                    </div>
                </div>
