...
describe("KeepRandomBeacon/RelayRequestCallback", function () {
  ...
  const operator = accounts[1] // make sure these match the ones in stakeAndGenesis.js
  const beneficiary = accounts[4]

  let entryVerificationFee
  let dkgContributionFee
  let groupProfitFee
  let baseCallbackGas
  ...
  before(async () => {
    ...
    const feeBreakdown = await serviceContract.entryFeeBreakdown()
  ...
  it("should revert when callback gas exceeds gas limit", async () => {
    ...
    const entryFeeEstimate = await serviceContract.entryFeeEstimate(2000000)
    await expectRevert(
  ...
  it("should produce entry when no callback was not provided", async () => {
    ...
    assert.equal(
      ...
      "Should emit event with the generated entry"
    )
  })
  ...
  it("should reimburse submitter when no callback was provided", async () => {
    ...
    const submitterReimbursement = beneficiaryEndBalance.sub(
  ...
  it("should produce entry and execute callback if provided", async () => {
    const callbackGas = await callbackContract.__beaconCallback.estimateGas(
    ...
    const result = web3.utils.toBN(await callbackContract.lastEntry())
    assert.isTrue(
  ...
  it("should reimburse submitter and customer for executing callback consuming less gas", async () => {
    let callbackGas = await callbackContract.__beaconCallback.estimateGas(
      blsData.groupSignature
    )
    ...
    const customerStartBalance = web3.utils.toBN(
      await web3.eth.getBalance(customer)
    )
    const beneficiaryStartBalance = web3.utils.toBN(
      await web3.eth.getBalance(beneficiary)
    )
    ...
    const relayEntryTxGasPrice = web3.utils.toBN(web3.utils.toWei("60", "Gwei"))
    await operatorContract.relayEntry(blsData.groupSignature, {
      from: operator,
      gasPrice: relayEntryTxGasPrice,
    })

    const customerEndBalance = web3.utils.toBN(
      await web3.eth.getBalance(customer)
    )
    const beneficiaryEndBalance = web3.utils.toBN(
      await web3.eth.getBalance(beneficiary)
    ...
    const customerSurplus = customerEndBalance.sub(customerStartBalance)
    const submitterReimbursement = beneficiaryEndBalance.sub(
      beneficiaryStartBalance
    )

    const gasSurplus = customerSurplus.div(relayEntryTxGasPrice)
  ...
  it("should reimburse submitter and customer for executing callback with lower tx.gasprice", async () => {
    ...
    const relayEntryTxGasPrice = web3.utils.toBN(web3.utils.toWei("2", "Gwei"))
  ...
  it("should reimburse submitter and customer for executing callback with higher tx.gasprice", async () => {
    ...
    const relayEntryTxGasPrice = web3.utils.toBN(web3.utils.toWei("70", "Gwei"))
    // higher tx.gasprice should not be used for reimbursement - maximum gas
    // price is the one from the gas price ceiling
    const gasPriceForReimbursement = web3.utils.toBN(
      web3.utils.toWei("60", "Gwei")
    )
  ...
  it("should trigger new group creation and execute callback if provided", async () => {
    ...
    assert.equal(
      ...
      "Should start group selection"
    )

    assert.equal(
      (await operatorContract.getPastEvents())[1].args["newEntry"].toString(),
      blsData.groupSignatureNumber.toString(),
      "Should start group selection with new entry"
    )
  ...
  // gas price ceiling == tx.gasprice
  it(
    "should trigger new group creation, execute callback, reimburse submitter " +
      "and customer with expected tx.gasprice",
    async () => {
      await fundDkgPool()
      ...
      const entryFeeEstimate = await serviceContract.entryFeeEstimate(
        callbackGas
      )
      await serviceContract.methods["requestRelayEntry(address,uint256)"](
        callbackContract.address,
      ...
      // use the same gas price as the gas price ceiling
      const relayEntryTxGasPrice = web3.utils.toBN(
        web3.utils.toWei("60", "Gwei")
      )
  ...
  // gas price ceiling > tx.gasprice
  it(
    "should trigger new group selection, execute callback, reimburse submitter " +
      "and customer with lower tx.gasprice",
    async () => {
      await fundDkgPool()
  ...
  it(
    "should trigger new group selection, execute callback, reimburse submitter " +
      "and customer with higher tx.gasprice",
    async () => {
      await fundDkgPool()
  ...
  it("should trigger new group creation when callback failed", async () => {
    ...
    const callbackGas = 1 // Requestor provides wrong gas
    const entryFeeEstimate = await serviceContract.entryFeeEstimate(callbackGas)
    await serviceContract.methods["requestRelayEntry(address,uint256)"](
      callbackContract.address,
      callbackGas,
  ...
  it("should reimburse submitter when callback failed", async () => {
    const estimate = await callbackContract.__beaconCallback.estimateGas(
      blsData.groupSignature
    )

    const callbackGas = estimate - 10 // Requestor provides wrong gas
    const entryFeeEstimate = await serviceContract.entryFeeEstimate(callbackGas)
    await serviceContract.methods["requestRelayEntry(address,uint256)"](
      callbackContract.address,
      callbackGas,
  ...
  it("should return surplus to requestor when callback fails", async () => {
    ...
    const callbackGas = estimate - 25000 // Requestor provides wrong gas
    const entryFeeEstimate = await serviceContract.entryFeeEstimate(callbackGas)
    await serviceContract.methods["requestRelayEntry(address,uint256)"](
      callbackContract.address,
      callbackGas,
    ...
    assert.isTrue(
      web3.utils
        ...
        .eq(dkgSubmitterReimbursementFee.add(groupProfitFee)),
  ...
  async function assertCallbackReimbursement(
    ...
    // around 'call'
    const expectedCallbackReimbursementGas = baseCallbackGas.addn(callbackGas)
    const actualCallbackReimbursementGas = submitterReimbursement
  ...
  async function assertTotalSpending(
    ...
    const totalSpentByBeacon = submitterReimbursement.add(customerSurplus)

    // calculate part the fee used for entry verification and callback
    const entryVerificationAndCallbackFee = web3.utils
      .toBN(entryFeeEstimate)
