...
describe("KeepRandomBeaconService/PricingDkg", () => {
  ...
  it(
    "should not trigger new group selection when there are not sufficient " +
      "funds in the DKG fee pool",
    async () => {
      ...
      const insufficientPoolFunds = web3.utils
        .toBN(groupCreationFee)
  ...
  it(
    "should trigger new group selection when there are sufficient funds in the " +
      "DKG fee pool",
    async () => {
      const entryFeeEstimate = await serviceContract.entryFeeEstimate(0)
      await serviceContract.methods["requestRelayEntry()"]({
  ...
  it("should not trigger group selection while one is in progress", async () => {
    ...
    const startBlock = await operatorContract.getTicketSubmissionStartBlock()

    await serviceContract.methods["requestRelayEntry()"]({
      value: entryFeeEstimate,
      from: accounts[0],
    })
    const contractBalance = web3.utils.toBN(
    ...
    assert.isTrue(
      web3.utils
        .toBN(await web3.eth.getBalance(serviceContract.address))
        .eq(contractBalance),
