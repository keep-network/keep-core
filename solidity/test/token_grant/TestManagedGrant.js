...
describe("TokenGrant/ManagedGrant", () => {
  ...
  const newGrantee = accounts[6]
  const anotherGrantee = accounts[7]
  const unrelatedAddress = accounts[8]
  ...
  before(async () => {
    ...
    stakeFromManagedGrant = async (
      ...
      return managedGrant.stake(staking.address, amount, delegation, {
        from: sender,
      })
    }
  })
  ...
  describe("changeReassignmentRequest", async () => {
    ...
    it("emits an event", async () => {
      ...
      expect(event.args["previouslyRequestedGrantee"]).to.equal(newGrantee)
      expect(event.args["newRequestedGrantee"]).to.equal(anotherGrantee)
    })
  })
  ...
  describe("confirmGranteeReassignment", async () => {
    ...
    it("emits an event", async () => {
      ...
      const event = (await managedGrant.getPastEvents())[0]
      expect(event.args["oldGrantee"]).to.equal(grantee)
      expect(event.args["newGrantee"]).to.equal(newGrantee)
    })
  })
