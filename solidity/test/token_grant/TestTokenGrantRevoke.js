...
describe("TokenGrant/Revoke", function () {
  ...
  it("should allow to revoke grant", async () => {
    const grantManagerKeepBalanceBefore = await tokenContract.balanceOf(
      tokenOwner
    )
    ...
    const withdrawableAtRevokedTimestamp = grantAmount
      .mul(revokedAt.sub(grantStart))
      .div(grantDuration)
  ...
  it("should be able to revoke the grant but no amount is refunded since duration of the unlocking is over.", async () => {
    ...
    const granteeGrantBalanceBefore = await grantContract.balanceOf.call(
      grantee
    )
    const grantManagerKeepBalanceBeforeRevoke = await tokenContract.balanceOf(
      tokenOwner
    )
    ...
    const grantManagerKeepBalanceAfterRevoke = await tokenContract.balanceOf(
      tokenOwner
    )
  ...
  it("should not be able to withdraw revoked tokens locked as stakes", async () => {
    ...
    expect(stakedAmount).to.eq.BN(
      minimumStake,
      "Minimum stake should be staked"
    )
