...
describe("KeepRandomBeaconOperator/PricingRewards", function () {
  ...
  it("should correctly evaluate delay factor right after the request", async () => {
    ...
    const delayFactor = await operatorContract.delayFactor.call()
  ...
  it("should correctly evaluate delay factor at the second submission block", async () => {
    ...
    await time.advanceBlockTo(
      web3.utils.toBN(await web3.eth.getBlockNumber()).addn(2)
    )
  ...
  it(
    "should correctly evaluate rewards for entry submitted " +
    ...
    async () => {
      ...
      const expectedGroupMemberReward = web3.utils.toBN("1410")
      const expectedSubsidy = web3.utils.toBN("0")
      ...
      // 10020 * 140000
      const expectedSubmitterReward = web3.utils.toBN("1402800000")

      const rewards = await operatorContract.getNewEntryRewardsBreakdown.call()
  ...
  it(
    ...
    async () => {
      ...
      // No delay so entire group member base reward is paid and nothing
      // goes to the subsidy pool.
      const expectedGroupMemberReward = web3.utils.toBN("966")
      const expectedSubsidy = web3.utils.toBN("0")
      // The entire entry verification fee is paid to the submitter
      // regardless of their gas expenditure. The submitter is free to spend
      // less or more, keeping the surplus or paying the difference:
      // 10050 * 150000
      const expectedSubmitterReward = web3.utils.toBN("1507500000")
  ...
  it(
    "should correctly evaluate rewards for the entry submitted " +
      "at the second submission block",
    async () => {
      ...
      const expectedGroupMemberReward = web3.utils.toBN("1976664")
      ...
      const expectedSubmitterReward = web3.utils.toBN("70070033073")
      ...
      const expectedSubsidy = web3.utils.toBN("628431")
  ...
  it(
    "should correctly evaluate rewards for the entry submitted " +
      "in the last block before timeout",
    async () => {
      ...
      const expectedGroupMemberReward = web3.utils.toBN("9372")
      ...
      const expectedSubmitterReward = web3.utils.toBN("104462370008")
      ...
      const expectedSubsidy = web3.utils.toBN("84025030184")
