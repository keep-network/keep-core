...
describe("KeepRandomBeaconOperator/ManageServiceContracts", () => {
  ...
  const serviceContractUpgrader = accounts[2]
  ...
  describe("addServiceContract", async () => {
    it("can be called by service contract upgrader", async () => {
      await operatorContract.addServiceContract(serviceContract2, {
    ...
    it("cannot be called by non-upgrader", async () => {
      await expectRevert(
        operatorContract.addServiceContract(serviceContract2, {
          from: someoneElse,
        }),
        "Not authorized"
      )
    ...
    it("adds service contract to the list", async () => {
      ...
      await operatorContract.sign(1, blsData.previousEntry, {
