...
describe("KeepRandomBeaconOperator/GroupTermination", function () {
  ...
  async function runTerminationTest(
    ...
    await time.advanceBlockTo(
      groupActiveTime.addn(await web3.eth.getBlockNumber())
    )
  ...
  describe("should fail when there are no active groups", async () => {
    it("T", async function () {
      await expectRevert(runTerminationTest(1, 0, [0], 0), "No active groups")
    })
    ...
    it("ET", async function () {
      await expectRevert(runTerminationTest(2, 1, [1], 0), "No active groups")
    })
  })
})
