...
const KeepRandomBeaconOperator = artifacts.require(
  "../stubs/KeepRandomBeaconOperatorRewardsStub.sol"
)
const KeepRandomBeaconServiceImpl = artifacts.require(
...
module.exports = async function () {
  try {
    ...
    const keepRandomBeaconService = await KeepRandomBeaconServiceImpl.at(
    ...
    await keepRandomBeaconService.methods["requestRelayEntry()"]({
    ...
    const allGroups = (
      await keepRandomBeaconOperator.getNumberOfCreatedGroups()
    ).toNumber()
    for (let i = 0; i < allGroups; i++) {
      const groupPublicKey = await keepRandomBeaconOperator.getGroupPublicKey(i)
      const isStale = await keepRandomBeaconOperator.isStaleGroup(
        groupPublicKey
      )
    ...
    async function registerNewGroups(numberOfGroups) {
      ...
      for (let i = 0; i < numberOfGroups; i++) {
        ...
        await keepRandomBeaconOperator.addGroupMemberReward(
          groupPublicKey,
          groupReward
        )
        console.log(
          "created group",
          await keepRandomBeaconOperator.getGroupPublicKey(i)
        )
      }
    }
  } catch (error) {
