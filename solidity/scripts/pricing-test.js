...
const KeepRandomBeaconOperatorStatistics = artifacts.require(
  "KeepRandomBeaconOperatorStatistics.sol"
)
const fs = require("fs")
...
module.exports = async function () {
  ...
  const contractService = await KeepRandomBeaconServiceImplV1.at(
  ...
  const accounts = operators.slice()
  ...
  for (;;) {
    try {
      ...
      const serviceContractSummary = new ServiceContractSummary(
        ...
        dkgFeePool.add(requestSubsidyFeePool).toString() ===
          serviceContractBalance
      )
      ...
      const callbackGas = 0
      const entryFeeEstimate = await contractService.entryFeeEstimate(
        callbackGas
      )
      const tx = await contractService.methods["requestRelayEntry()"]({
      ...
      const txGasCost = tx.receipt.gasUsed
      const txCost = web3.utils.toBN(gasPrice).mul(web3.utils.toBN(txGasCost))

      requestorAccountBalance = await web3.eth.getBalance(requestor)

      const requestorAccountBalanceChange = web3.utils
        .toBN(requestorAccountBalance)
        .sub(web3.utils.toBN(requestorPrevAccountBalance))
        .toString()
      ...
      for (let i = 0; i < accounts.length; i++) {
        ...
        const balanceChange = web3.utils
          .toBN(balance)
          .sub(web3.utils.toBN(prevBalances[i]))
          .toString()

        const reward = (
          await availableRewards(address, contractOperator, contractStatistics)
        ).toString()
        const rewardChange = web3.utils
          .toBN(reward)
          .sub(web3.utils.toBN(prevRewards[i]))
          .toString()
...
async function availableRewards(account, contractOperator, contractStatistics) {
  const expiredGroupCount = (
    await contractOperator.getFirstActiveGroupIndex()
  ).toNumber()
  ...
  for (let groupIndex = 0; groupIndex < totalGroupCount; groupIndex++) {
    groupsPublicKeys[groupIndex] = await contractOperator.getGroupPublicKey(
      groupIndex
    )
  }
  ...
  for (let i = 0; i < groupsPublicKeys.length; i++) {
    const groupMembersCount = await contractStatistics.countGroupMembership(
      groupsPublicKeys[i],
      account
    )
    const groupMemberReward = await contractOperator.getGroupMemberRewards(
      groupsPublicKeys[i]
    )
    accountRewards = accountRewards.add(
      groupMembersCount.mul(groupMemberReward)
    )
  }
...
function ServiceContractSummary(
  ...
  ;(this.balance = balance), (this.dkgFeePool = dkgFeePool)
