...
module.exports = async function () {
  ...
  async function stake() {
    ...
    console.log(
      `Staking ${amountToStake} tokens from ${owner} to operator ${operator} using beneficiary ${beneficiary} and authorizer ${authorizer}`
    )
    ...
    try {
      ...
      console.log(
        `could not authorize operator contract for ${operator}: ${err}`
      )
    }
  }
  ...
  async function printStakeInfo() {
    try {
      ...
      console.log(
        `KEEP tokens available:  ${(await getOwnerBalance()).toString()}`
      )
      console.log(
        `KEEP eligible stake:    ${(await getEligibleStake()).toString()}`
      )
      console.log(
        `KEEP active stake:      ${(await getActiveStake()).toString()}`
      )
      console.log(
        `Minimum stake:          ${(await getMinimumStake()).toString()}`
      )
    } catch (err) {
      console.log(err)
    }
  ...
  async function getEligibleStake() {
    return tokenStaking.eligibleStake(
      operator,
      keepRandomBeaconOperator.address
    )
  }
