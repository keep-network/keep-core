...
Eligible group member - see <<fees-and-rewards,Fees and Rewards>> section -
submits the result to the chain calling
...
`RandomBeaocn.submitRelayEntry(bytes calldata entry)`
function.
...
the sortition pool rewards for a governable time period. The submitter of the failed
...
funded by DAO with tokens saved from sortition pool rewards as a result of
having operators marked as ineligible for rewards due to failing a heartbeat.
