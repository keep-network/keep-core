...
the bootstrap peer connects peer to the network and announces peer's presence. 
...
in the network. Before the peer decides to connect to any other network member, the 
same nonce, challenge, and stake check protocol must be executed between the peer
and the new network member it is connecting to. 
...
split brain event. Just like it is with the network join protocol, nonce and challenge 
are discarded when the connection protocol is completed.
