...
kubectl create secret generic terraform-admin-service-account \
  --from-file=/Users/sthompson22/.config/gcloud/thesis-terraform-admin-service-account-creds.json
#+END_SRC
...
Default backend:  atlantis-https:8443 (10.102.1.14:8443)
TLS:
  atlantis-tls terminates atlantis.keep-dev.com
Rules:
  Host  Path  Backends
  ----  ----  --------
  *     *     atlantis-https:8443 (10.102.1.14:8443)
Annotations:
  ingress.kubernetes.io/backends:               {"k8s-be-30885--2233353f8ad2173d":"Unknown"}
...
Events:
  Type    Reason  Age    From                     Message
  ----    ------  ----   ----                     -------
  Normal  ADD     2m36s  loadbalancer-controller  default/atlantis-https
  Normal  CREATE  117s   loadbalancer-controller  ip: 35.244.176.50
:END:
...
kubectl create secret generic atlantis-git \
  --from-file=./secrets/atlantis-git/gh-webhook-secret \
  --from-file=./secrets/atlantis-git/gh-access-token
#+END_SRC
...
kubectl create secret tls atlantis-tls \
  --cert=./secrets/atlantis-tls/atlantis.keep-dev.crt \
  --key=./secrets/atlantis-tls/atlantis.keep-dev.key
#+END_SRC     
